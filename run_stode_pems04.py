...
def eval(loader, model, std, mean, device):
    ...
    for idx, (inputs, targets) in enumerate(tqdm(loader)):
        ...
        # print(rmse_loss.shape, mae_loss.shape)
        # 计算每个批次的平均损失值，并将其添加到列表中
        mae_losses.append(np.mean(mae_loss, axis=(0,2)))
        rmse_losses.append(np.mean(rmse_loss, axis=(0,2)))
        mape_losses.append(np.mean(mape_loss, axis=(0,2)))

    # 计算所有批次的平均损失值
    if mae_losses:
        mean_mae_loss = np.mean(mae_losses, axis=0)
        mean_rmse_loss = np.mean(rmse_losses, axis=0)
        mean_mape_loss = np.mean(mape_losses, axis=0)
    else:
        mean_mae_loss = np.array([])
        mean_rmse_loss = np.array([])
        mean_mape_loss = np.array([])

    return mean_rmse_loss, mean_mae_loss, mean_mape_loss
...
def run_model(data, mean, std, dtw_matrix, sp_matrix, dataset_name, args, device):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if valid_rmse.mean() < best_valid_rmse:
            ...
            # 保存模型参数
            model_path = f'F:/AAA/STGODE/data/PEMS04/net_params_{os.path.basename(dataset_name)}_{args.num_gpu}.pkl'
        ...
        if args.log:
            ...
            # for i, mae in enumerate(train_mae):
            #     print(f"Column {i + 1}: {mae}")
            # print("##on train data## mape loss:")
            # for i, mape in enumerate(train_mape):
            #     print(f"Column {i + 1}: {mape}")
    ...
    net.load_state_dict(torch.load(f'F:/AAA/STGODE/data/PEMS04/net_params_{os.path.basename(dataset_name)}_{args.num_gpu}.pkl'))
...
def main(args):
    ...
    data1, mean1, std1, dtw_matrix1, sp_matrix1 = read_data(args, 'F:/AAA/STGODE/data/PEMS04')
    test_rmse1, test_mae1, test_mape1 = run_model(data1, mean1, std1, dtw_matrix1, sp_matrix1, 'PEMS04', args, device)

    data2, mean2, std2, dtw_matrix2, sp_matrix2 = read_data(args, 'F:/AAA/STGODE/data/PEMS04/PEMS04_STL')
    test_rmse2, test_mae2, test_mape2 = run_model(data2, mean2, std2, dtw_matrix2, sp_matrix2, 'PEMS04_STL', args,device)
    ...
    for i in range(num_figures):
        ...
        x = range(start, end)
        width = 0.25
        plt.bar(x, test_rmse1[start:end], width, label='PEMS04', color='#B8D4E9')
        plt.bar([p + width + 0.05 for p in x], test_rmse2[start:end], width, label='PEMS04_STL', color='#2F7DBB')
        plt.xlabel('Column')
        ...
        plt.tight_layout()
    # 保存图像
        plt.savefig(f'F:/AAA/STGODE/data/PEMS04/result/PEMS04_test_rmse_comparison_{i}.png')
        plt.show()

    # 创建一个包含两个数据集测试RMSE的DataFrame
    df_results = pd.DataFrame({
        'EIA24 Test RMSE': test_rmse1,
        'EIA24 Test MAE': test_mae1,
        'EIA24 Test MAPE': test_mape1,
        'EIA24_STL Test RMSE': test_rmse2,
        'EIA24_STL Test MAE': test_mae2,
        'EIA24_STL Test MAPE': test_mape2
    })

    # 保存DataFrame到Excel文件
    df_results.to_excel('F:/AAA/STGODE/data/PEMS04/result/PEMS04_test.xlsx', index=False)
