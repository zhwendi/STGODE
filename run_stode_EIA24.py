...
def run_model(data, mean, std, dtw_matrix, sp_matrix, dataset_name, args, device):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if valid_rmse.mean() < best_valid_rmse:
            ...
            # 保存模型参数
            model_path = f'F:/AAA/STGODE/data/EIA24/net_params_{os.path.basename(dataset_name)}_{args.num_gpu}.pkl'
    ...
    net.load_state_dict(torch.load(f'F:/AAA/STGODE/data/EIA24/net_params_{os.path.basename(dataset_name)}_{args.num_gpu}.pkl'))
...
def main(args):
    ...
    data1, mean1, std1, dtw_matrix1, sp_matrix1 = read_data(args, 'F:/AAA/STGODE/data/EIA24/EIA24')
    test_rmse1, test_mae1, test_mape1 = run_model(data1, mean1, std1, dtw_matrix1, sp_matrix1, 'EIA24', args, device)

    data2, mean2, std2, dtw_matrix2, sp_matrix2 = read_data(args, 'F:/AAA/STGODE/data/EIA24/EIA24_STL')
    test_rmse2, test_mae2, test_mape2 = run_model(data2, mean2, std2, dtw_matrix2, sp_matrix2, 'EIA24_STL', args,device)

    # Plotting the test RMSE comparison bar chart
    plt.figure(figsize=(10, 6))
    x = range(len(test_rmse1))
    width = 0.35  # 条形图的宽度
    plt.bar(x, test_rmse1, width, label='EIA24', color='#B8D4E9')
    plt.bar([p + width + 0.05 for p in x], test_rmse2, width, label='EIA24_STL', color='#2F7DBB')
    plt.xlabel('Column')
    ...
    plt.tight_layout()
    # 保存图像
    plt.savefig('F:/AAA/STGODE/data/EIA24/EIA24_test_rmse_comparison.png')
    plt.show()
    ...
    # 保存DataFrame到Excel文件
    df_results.to_excel('F:/AAA/STGODE/data/EIA24/EIA24_test_loss.xlsx', index=False)
