...
class TemporalConvNet(nn.Module):
    ...
    def __init__(self, num_inputs, num_channels, kernel_size=2, dropout=0.2):
        ...
        for i in range(num_levels):
            ...
            # self.conv = nn.Conv2d(1, out_channels, (1, kernel_size), dilation=(1, dilation_size), padding=(0, padding))
        ...
        # self.downsample = nn.Conv2d(1, num_channels[-1], (1, 1)) if num_inputs != num_channels[-1] else None
        if self.downsample:
    ...
    def forward(self, x):
        ...
        # print("x",x.shape,x)    #[112,13,13,1]        [16,307,12,3]
        y = x.permute(0, 3, 1, 2)
        # print("y",y.shape,y)    #[112,1,13,13]         [16,3,307,12]
        # y =F.relu(self.network(y) + self.downsample(y) if self.downsample else y)
        y = F.leaky_relu(self.network(y) + self.downsample(y) if self.downsample else y)
        # print("y",y.shape,y)  #[112,64,13,13]          [16,64,307,12]
        y = y.permute(0, 2, 3, 1)
        # print("y",y.shape,y)    #[112,13,13,64]        [16,307,12,64]
...
class GCN(nn.Module):
    ...
    def forward(self, X):
        ...
        return torch.tanh(torch.einsum('kjlm, mn->kjln', y, self.theta))
...
class STGCNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, num_nodes, A_hat):
        ...
        self.odeg = ODEG(out_channels[-1], 24, A_hat, time=6)
    ...
    def forward(self, X):
        ...
        t = self.temporal2(F.leaky_relu(t))
